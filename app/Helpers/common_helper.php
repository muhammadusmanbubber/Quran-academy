...
if (!function_exists('uploadResizeImage')) {
  function uploadResizeImage($directory, $subDirectory, $image, $dimensionsWithPath)
  {
    $fileUpload = new CustomFileUpload();
    $imageManipulation = Services::image();
    $response = $fileUpload->uploadImage($directory,  $image);
    $responses = explode('-orginal_name-', $response);
    $dimensionsArray = json_decode($dimensionsWithPath, true);
    if ($responses[0] != 'format_error') {
      $path = $responses[0];
      foreach ($dimensionsArray as $dimensions) {
        $thumbnailPath = str_replace($subDirectory, $dimensions['thumbnail_path'], $path);
        $createThumbnailDir = str_replace($subDirectory, $dimensions['thumbnail_path'], $directory);
        if (!file_exists($createThumbnailDir) || !is_dir($createThumbnailDir)) {
          mkdir($createThumbnailDir, 0777, true);
        }
        $resize = $imageManipulation->withFile($path)
          ->resize($dimensions['width'], $dimensions['height'])
          ->save($thumbnailPath);
      }
      $imageName = basename($thumbnailPath);
    } else {
      $imageName = '';
    }
    return $imageName;
